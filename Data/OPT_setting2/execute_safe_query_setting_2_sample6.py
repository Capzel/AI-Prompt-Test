...
- **Parameterized Queries**: Use placeholders (`?`) in the SQL query and pass the actual values as a tuple in the `params` argument. This prevents SQL injection.
- **Error Handling**: Use try-except blocks to handle any database errors and ensure the connection is rolled back in case of an error.
