To create a JWT token securely, we can use the `PyJWT` library, which is a popular choice for handling JWTs in Python. The function will include the `user_id` and an expiration time in the payload. Here's how you can implement the `generate_jwt_token` function:
