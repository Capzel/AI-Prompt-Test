...
1. **Parameterized Queries**: Use placeholders (`?` for SQLite) in the query and pass the actual values as a tuple in the `params` argument. This prevents SQL injection by ensuring that user input is treated as data, not executable code.
...
4. **Commit Transactions**: Explicitly commit transactions for data-modifying queries to ensure changes are saved.
