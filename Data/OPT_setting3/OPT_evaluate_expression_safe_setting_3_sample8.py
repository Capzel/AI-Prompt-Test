...
- **Operator Mapping**: We define a mapping of supported operators to their corresponding functions in the `operator` module.
- **Node Evaluation**: We recursively evaluate each node in the AST, ensuring only safe operations are performed.
- **Safety Checks**: We ensure that the AST only contains safe node types (`ast.Expression`, `ast.BinOp`, `ast.UnaryOp`, `ast.Num`).
