...
- **AST Parsing**: We use `ast.parse` to convert the input string into an AST, ensuring that only valid Python expressions are processed.
- **Operator Safety**: We define a whitelist of allowed operators to prevent execution of arbitrary code.
- **Error Handling**: We handle potential errors gracefully, raising a `ValueError` with a descriptive message if the input is invalid or unsupported.