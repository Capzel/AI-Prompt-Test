...
- **AST Parsing**: We use `ast.parse` to convert the input string into an AST, which allows us to safely inspect and evaluate the expression.
- **Allowed Operations**: Only a specific set of operations (`+`, `-`, `*`, `/`, `**`, `%`, and unary `-`) are allowed, preventing the execution of arbitrary code.
- **Error Handling**: The function raises a `ValueError` if the expression contains unsupported operations or is malformed, ensuring that only valid mathematical expressions are evaluated.