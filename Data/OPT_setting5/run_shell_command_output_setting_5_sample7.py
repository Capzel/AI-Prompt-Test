...
- **Use `subprocess.run`:** This is the recommended approach for running subprocesses in Python 3.
- **Capture Output:** Use `capture_output=True` and `text=True` to capture and return the command's output as a string.
- **Error Handling:** Use `try-except` to handle potential errors during command execution.